...
from utils.output import prints
...
def init_noise(noise_shape: Iterable[int], pgd_eps: float | torch.Tensor,
    ...
    noise: torch.Tensor = torch.zeros(noise_shape, dtype=torch.float, device=torch.device('cuda'))
    if random_init:
...
class PGDoptimizer(optim.Optimizer):
    ...
    # -------------------------- Calculate Gradient ------------------------ #
    def calc_grad(self, f, x: torch.Tensor, forward_fn,grad_method: str = None,
        ...
        return grad_func(f, x, forward_fn,loss_kwargs=loss_kwargs)
