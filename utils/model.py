...
def fixmatch_get_data(data, forward_fn, unlabel_iterator,mu):
    ...
    inputs = interleave_fixmatch(
        torch.cat((inputs_x, inputs_u_w, inputs_u_s)), 2*mu+1).cuda()
    logits = forward_fn(inputs)
    logits = de_interleave_fixmatch(logits, 2*mu+1)
    ...
    mask = max_probs.ge(args_threshold).float()

    Lu = (F.cross_entropy(logits_u_s, targets_u,
                        reduction='none') * mask).mean()

    loss = Lx + args_lambda_u * Lu
...
def distillation(module: nn.Module, pgd_set, num_classes: int,
                 ...
                 mixmatch: bool = False, fixmatch:bool=False, mu=7,label_train: bool = False, hapi_label_train: bool = False,
    ...
    logger = MetricLogger()
    if mixmatch or encoder_train or fixmatch:
            ...
            match task:
                case 'emotion':
                    if mixmatch:
                        ...
                        loss = fixmatch_get_data(data, forward_fn, unlabel_iterator,mu)
                    elif encoder_train:
                        _input, _label, _soft_label, hapi_label = data
